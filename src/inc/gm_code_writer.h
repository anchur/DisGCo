...
public:
    ...
    void set_ready_to_push() {
      ready_to_push = true;
    }

    void reset_ready_to_push() {
      ready_to_push = false;
    }
...
public:
    ...
    void insert_at(int ptr, int then_indent, const char* str) {
      if(ready_to_push) {
        printf("in insert_at ptr= %d\n", ptr);
        printf("in insert_at file_ptr= %d\n", file_ptr);
        assert(str!=NULL);
        assert(ptr < file_ptr);
		...
		int get_prev_line_ptr() {
			return file_ptr - 1;
		}

		void insert_at_prev_line(const char *str) {
      if(ready_to_push) {
        int tmp_col = col;
        col = 0;
        int prev_line_ptr = get_prev_line_ptr();
        insert_at(prev_line_ptr, indent, "\n");
        prev_line_ptr = get_prev_line_ptr();
        insert_at(prev_line_ptr, indent, str);
        col = tmp_col;
      }
		}
};
