...
public:
    ...
    virtual void add_mpi_map_def(ast_maptypedecl* map, ast_id* mapId);
...
public:
    ...
    void get_tempvar_name_and_type(const char **, const char **, ast_field *);
    virtual void generate_sent_nop(ast_nop* n);
...
protected:
    ...
    //virtual void generate_mpi_get_block(int, int);
    //virtual void generate_mpi_put_block(ast_field *f);
#ifdef __USE_BSP_OPT__
    virtual void generate_opt_body(ast_assign * a, const char * rhs_temp);
    void generate_opt_inits();
    void emit_post_opt_body(GM_REDUCE_T);
#endif 
...
//-----------------------------------
DEF_STRING(LABEL_DECLARED_IN_PAR);
