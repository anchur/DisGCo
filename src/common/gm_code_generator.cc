...
void gm_code_generator::generate_mapaccess(ast_expr_mapaccess* e) {
    ...
    if (MPI_GEN.is_in_parallel_loop()) {
      sprintf(buffer, "%s.getValue_par(", map->get_genname());
    } else {
      sprintf(buffer, "%s.getValue_seq(", map->get_genname());
    }
    _Body.push(buffer);
    generate_expr(key);
    _Body.push(")");
...
#endif
//TODO: these variables has to be saved and restored in case of nested loops.
#ifdef __USE_BARRIER_OPT
    if(MPI_GEN.s_w) {
      MPI_GEN.s_w = false;
      MPI_GEN.barrier_pt_set = false;
      MPI_GEN.curr_ind_BO = 0;
      MPI_GEN.f_ptr_BO = _Body.get_write_ptr(MPI_GEN.curr_ind_BO);
      MPI_GEN.found_atleat_a_prop = false;
    }
#endif
...
void gm_code_generator::generate_sent_if(ast_if *i) {
	...
	// using MPI_Get.
	int curr_indent = 0;
	int fp = _Body.get_write_ptr(curr_indent);	
	...
	_Body.pushln(") {");
#ifdef __USE_BARRIER_OPT
  vector<const char *> * rprops_before_if = MPI_GEN.read_props_before_barrier;
  vector<const char *> * wprops_before_if = MPI_GEN.written_props_before_barrier;
  MPI_GEN.read_props_before_barrier = new vector<const char *>();
  MPI_GEN.written_props_before_barrier = new vector<const char *>();
  MPI_GEN.copyprops(rprops_before_if, wprops_before_if);
#endif
  ...
  if (MPI_GEN.is_in_parallel_loop()) {
	...
	{
		MPI_GEN.generate_mpi_get_block(fp, curr_indent, _Body);
  ...
  _Body.pushln("}");
	...
	if (s == NULL){ 
#ifdef __USE_BARRIER_OPT
    MPI_GEN.union(MPI_GEN.read_props_before_barrier, rprops_before_if);
    MPI_GEN.union(MPI_GEN.written_props_before_barrier, wprops_before_if);
#endif
    return;
  }
	...
	_Body.push("else {");
#ifdef __USE_BARRIER_OPT
  vector<const char *> * rprops_after_if = MPI_GEN.read_props_before_barrier;
  vector<const char *> * wprops_after_if = MPI_GEN.written_props_before_barrier;
  MPI_GEN.read_props_before_barrier = new vector<const char *>();
  MPI_GEN.written_props_before_barrier = new vector<const char *>();
  MPI_GEN.copyprops(rprops_before_if, wprops_before_if);
#endif
...
#endif
#ifdef __USE_BARRIER_OPT
    MPI_GEN.union(MPI_GEN.read_props_before_barrier, rprops_after_if);
    MPI_GEN.union(MPI_GEN.written_props_before_barrier, wprops_after_if);
#endif
...
void gm_code_generator::generate_sent_while(ast_while *w) {
  ...
  if (MPI_GEN.is_in_parallel_loop()) {
    ...
    if (w->is_do_while()) {
      ...
      strcat(tmp_write_buffer, "MPI_Barrier(MPI_COMM_WORLD;)");
      Body.insert_at(MPI_GEN.f_ptr_BO, MPI_GEN.curr_ind_BO, tmp_write_buffer);
#endif
        generate_sent(b);
    }
