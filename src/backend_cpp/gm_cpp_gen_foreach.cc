...
void gm_cpplib::generate_down_initializer(ast_foreach* f, gm_code_writer& Body) {
    ...
    if (gm_is_simple_collection_iteration(iter_type) || gm_is_collection_of_collection_iteration(iter_type)) {
        ...
        if (gm_is_collection_of_collection_iteration(iter_type)) {
        	...
        	Body.push(type_name);
        	Body.push(" ");
        	Body.push(f->get_iterator()->get_genname());
        	Body.pushln(";");
        	Body.pushln("if(G.get_rank() == MASTER)");
        	Body.pushln("{");
        	sprintf(str_buf, "%s = %s.get_next();", f->get_iterator()->get_genname(), lst_iter_name);
        	Body.pushln(str_buf);
        	Body.pushln("}");
        	if(strcmp(type_name,"node_t")==0)
        	{
        		Body.push("MPI_Bcast(&");
        		Body.push(f->get_iterator()->get_genname());
        		Body.push(", 1, MPI_INT, MASTER, MPI_COMM_WORLD);");
        		Body.NL();
        ...
        if (gm_is_down_nbr_node_iteration(iter_type)) {
           ...
           //mpi
            Body.pushln(str_buf);
            //mpi
            /*if (MPI_GEN.get_in_bfs_body()) {
            	char *neighbor_array_get_func = "get_t_neighbor";
            	sprintf(str_buf, "%s %s = %s (%s);", type_name, var_name, neighbor_array_get_func, alias_name);
            } else {*/
            ...
            sprintf(str_buf, "%s %s = %s [%s-%s];", type_name, var_name, "new_node_idx", alias_name, "new_begin[0]");
            }
            Body.pushln(str_buf);
        }
    }
...
void gm_cpplib::generate_foreach_header(ast_foreach* fe, gm_code_writer& Body) {
    ...
    if (gm_is_all_graph_iteration(type)) {
        ...
        //considering this for loop as a parallel loop. If not parallel, the following code will be wrong.
        //mpi
        if (fe->is_parallel()) {
          if(gm_is_node_iteration(type))
            MPI_GEN.local_access_nodes.push_back(it_name);
          else
            MPI_GEN.local_access_edges.push_back(it_name);
        }
        if (fe->is_parallel()) {
#ifdef __USE_BSP_OPT__
            if(MPI_GEN.is_bsp_nested_2()){
              //if there is a triply nested loop, the loop is not bsp canonical.
              MPI_GEN.reset_bsp_nested_1();
              MPI_GEN.reset_bsp_nested_2();
              MPI_GEN.reset_bsp_canonical();
            }
            else if (!MPI_GEN.is_bsp_nested_1()) {
              //if it is the first loop iterating over nodes or edges, 
              //set first level nesting to true.
              MPI_GEN.set_bsp_nested_1();
            }
            else{
              //TODO: set an impossible flag
            }
#endif
          sprintf(str_buf, "for (%s %s = 0; %s < %s.%s(); %s ++) ", get_type_string(iter->getTypeSummary()), it_name, it_name, graph_name,
              gm_is_node_iteration(type) ? NUM_LOCAL_NODES : NUM_LOCAL_EDGES, it_name);
        } else {
          sprintf(str_buf, "for (%s %s = 0; %s < %s.%s(); %s ++) ", get_type_string(iter->getTypeSummary()), it_name, it_name, graph_name,
        ...
        if (MPI_GEN.is_in_parallel_loop()) {
          if (MPI_GEN.check_in_vars_in_scope(it_name)) {
            MPI_GEN.remove_var(it_name);
          }
        }
#endif
        Body.pushln(str_buf);
        ...
        //mpi
        /*if (MPI_GEN.get_in_bfs_body()) {
        	char *neighbor_begin = "get_t_neighbors_begin";
        	char *neighbor_end = "get_t_neighbors_end";
        	sprintf(str_buf, "for (%s %s = %s();", EDGE_T, alias_name, neighbor_begin);
        	Body.push(str_buf);
        	sprintf(str_buf, "%s < %s(); %s ++) ", alias_name, neighbor_end, alias_name);
        	Body.pushln(str_buf);
        } else {*/
          //needed modification here
        if(MPI_GEN.is_present_in_local_access_nodes(src_name)) {
          MPI_GEN.local_access_edges.push_back(alias_name);
         
#ifdef __USE_BSP_OPT__
          if(MPI_GEN.is_bsp_nested_1()) {
            if(!MPI_GEN.is_bsp_nested_2()) {
              MPI_GEN.set_bsp_nested_2();
              MPI_GEN.set_bsp_canonical();
            }else {
              MPI_GEN.reset_bsp_nested_1();
              MPI_GEN.reset_bsp_nested_2();
              MPI_GEN.reset_bsp_canonical();
          ...
          //begin and node_idx need to be get.
					//need to be verified
					Body.pushln("edge_t * new_begin = (edge_t *)calloc(2, sizeof(edge_t));");
					Body.push("lock_and_get_begin(new_begin[0], ");
          Body.push(src_name); 
          Body.pushln(", 2, G);");
					Body.pushln("int node_idx_size = new_begin[1] - new_begin[0];");
					Body.pushln("node_t * new_node_idx = (node_t *)calloc(node_idx_size, sizeof(node_t));");
					Body.push("lock_and_get_node_idx(new_node_idx[0], ");
          Body.push(src_name); 
          Body.pushln(", new_begin[0], node_idx_size, G);");
					Body.push("adjust_begin_to_global_values(new_begin, "); 
          Body.push(src_name);
          Body.pushln(", G);");
          sprintf(str_buf, "for (%s %s = %s[0];", EDGE_T, alias_name, "new_begin");
          Body.push(str_buf);
          sprintf(str_buf, "%s < %s[1] ; %s ++) ", alias_name,  "new_begin", alias_name);
          Body.pushln(str_buf);
        ...
        Body.push("while ( ");
        Body.push(fe->get_source()->get_genname());
        Body.push("_has_next )");
