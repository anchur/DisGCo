...
void gm_cpplib::add_mpi_map_def(ast_maptypedecl* map, ast_id* mapId) {
  ...
  if(MPI_GEN.is_in_parallel_loop()) {
    char typeBuffer[128];
    sprintf(typeBuffer, "gm_map_medium");

    //currently we dont have any default values.
    ...
    char buffer[256];
    sprintf(buffer, "%s<%s, %s> %s (%s, %s);", typeBuffer, "node_t", "int32_t", mapId->get_genname(), "gm_rt_get_num_threads()", "0");
    Body->pushln(buffer);
  }else {
    char typeBuffer[128];

    sprintf(typeBuffer, "gm_map_node_int");
    ...
    char buffer[256];
    sprintf(buffer, "%s %s;", typeBuffer, mapId->get_genname());
    Body->pushln(buffer);
  }
}
...
const char* gm_cpplib::get_function_name_map_par(int methodId) {
    switch (methodId) {
        //mpi
        //changed _seq to _par in all the below returns.
        //mpi
        case GM_BLTIN_MAP_HAS_MAX_VALUE:
            ...
            switch (method_id) {
                case GM_BLTIN_NODE_DEGREE:
                	Body.push("/*1*/");
										//mpi
									  char temp_buf[256];
                    if(MPI_GEN.is_present_in_local_access_nodes(i->get_genname())){
										  sprintf(temp_buf, "int32_t ___begin_start = G.local_begin[%s];int32_t  ___begin_end = G.local_begin[%s+1];",i->get_genname(), i->get_genname());
                    }
                    else {
										  sprintf(temp_buf, "int ___begin_start, ___begin_end; get_begin_start_end(%s, ___begin_start, ___begin_end, G);",
											  	i->get_genname());
                    }
									  Body.insert_at_prev_line(temp_buf);
                    ...
                    sprintf(str_buf, "(___begin_end - ___begin_start)");
										//mpi
                    Body.push(str_buf);
                    break;
        ...
        case GMTYPE_NODE_ITERATOR:
            switch (method_id) {
                case GM_BLTIN_NODE_TO_EDGE: {
                    ...
                    if(MPI_GEN.is_present_in_local_access_edges(alias_name)){
                    if(!MPI_GEN.is_assign_lhs_scalar()) {
                      sprintf(str_buf, "G.get_global_edge_num(%s)",alias_name);
        ...
        case GMTYPE_EDGE:
            switch (method_id) {
                ...
                case GM_BLTIN_EDGE_TO: {
                    //may be a check whether i->get_genname() is in local_access_nodes; if not, generate new_node_idx
                    // following need to be verified
                    Body.push("/*reaching2*/");
                    if(MPI_GEN.is_present_in_local_access_nodes(i->get_genname())||MPI_GEN.is_present_in_local_access_edges(i->get_genname())) {
                      sprintf(str_buf, "%s.%s[%s]", i->getTypeInfo()->get_target_graph_id()->get_genname(), NODE_IDX, i->get_genname());
                    } else {
                      sprintf(str_buf, "%s[%s - %s]", "new_node_idx", i->get_genname(), "new_begin[0]");
                    }
                }
                    break;
                default:
