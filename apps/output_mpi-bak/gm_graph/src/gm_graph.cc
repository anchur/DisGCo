...
//mpi
#define MASTER 0
#define NUM_BASE_ADDRESS_ENTRIES 9
...
void gm_graph::distribute() {
  ...
  // Broadcast _numNodes and _numEdges to all processors.
	...
	_num_of_local_nodes = _n / block_size;
	...
	if (rank == MASTER) {
		...
		for (proc_id = 0; proc_id < num_processes; proc_id++) {
			...
			num_of_local_forward_edges_for_proc_id =
				begin[proc_id * block_size + 1] - begin[proc_id * block_size];
			// step 4
			// step 5
			global_forward_edge_start_for_proc_id = begin[proc_id * block_size];
			//step 5
			num_of_local_reverse_edges_for_proc_id =
				r_begin[proc_id * block_size + 1] - r_begin[proc_id * block_size];
	...
	if(rank == MASTER)
	{
		...
		for (proc_id = 0; proc_id < num_processes; proc_id++) {
		  ...
		  // having elements from begin[proc_id * block_size] to
			...
			MPI_Put(local_r_node_idx_for_proc_id, num_of_local_reverse_edges_for_proc_id + 1, MPI_INT, proc_id, base_address[7*num_processes + proc_id], num_of_local_reverse_edges_for_proc_id, MPI_INT, win);
			MPI_Put(local_e_rev2idx_for_proc_id, num_of_local_reverse_edges_for_proc_id + 1, MPI_INT, proc_id, base_address[8*num_processes + proc_id], num_of_local_reverse_edges_for_proc_id, MPI_INT, win);
