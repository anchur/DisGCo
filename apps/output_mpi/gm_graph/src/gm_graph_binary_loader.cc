...
{
    clear_graph();
    //anchu lifted up the declarations
    int *Vindex, *Pindex, *procMap, numP;
    int pStatus = 0;
    int prevProc = 0;
    edge_t old_key = 0;
    edge_t *num_prev_edges;
    int32_t key;
    int i;
...
#define BITS_TO_EDGE(X) ((old_flipped_format)? (X) : (saved_edge_t_size == 4) ? n32tohedge(X) : n64tohedge(X))
    ...
    pStatus = read_partition_summary(&(this->blk_size), &Vindex, &Pindex, &numP);
    if (pStatus > 0) {
      fprintf(stderr, "Error reading partition file");
      goto error_return;
    }


    if (pStatus == 0){
      procMap = (int *)malloc(numP*sizeof(int)); 
      memcpy(procMap,Pindex,numP*sizeof(int)); //copy the edge boundaries.
      num_prev_edges = (edge_t *)calloc(numP, sizeof(edge_t));
    }else {
     int num_processes = this -> get_num_processes();

     int block_size = N/num_processes;
     int p_id;
     printf("%d is N %d is num_procs\n",N,num_processes);
     this ->blk_size = (node_t *) calloc(num_processes, sizeof(node_t));
     for(p_id = 0; p_id<num_processes; p_id++) {
       if(p_id == num_processes-1) {
         this->blk_size[p_id] = N - (p_id * block_size);
       } else {
         this->blk_size[p_id] = block_size;  
       }
     }
    }
#if GM_GRAPH_NUMA_OPT 
    ...
    //edge_t *num_prev_edges = (edge_t *)calloc(numP, sizeof(edge_t));
    for (node_t i = 0; i < N + 1; i++) {
      ...
      // begin 0, 1, 3, 4, 6
      //
      // target begin 0, 1, 2, 4, 6
      if (pStatus == 0 && i != N){
        p = nextProcIndex(); 				// p = 0
        new_index = Vindex[p]++;			// new_index = 2, Vindex = [3, 4]
        vmap[i]=new_index;				// 
        printf("VMAP[%d] = %d\n", i, new_index);
        edge_t t = key;					// t = 4
        num_prev_edges[prevProc] = key-old_key;		// num_prev_edges [1, 2]
        key = procMap[p]+num_prev_edges[p];		// key = 2
        procMap[p] = key;				// procMap [2, 4]
        prevProc=p;					// prevProc = 0
        old_key = t;					// old_key = 4
      }
#if GM_GRAPH_NUMA_OPT
      temp_begin[new_index] = key;
#else
      this->begin[new_index] = key;
#endif
    }
    ...
    read_partition_summary(&(this->blk_size), &Vindex, &Pindex, &numP); // reset partition file pointer.
    ...
    printf("EMAP[%d] = %d\n", i, new_index);
#if GM_GRAPH_NUMA_OPT
        temp_node_idx[new_index] = key;
#else
        this->node_idx[new_index] = key;
#endif
    }
