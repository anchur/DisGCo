...
#pragma omp parallel for
    ...
    void enter_node(node_t n) {
        ...
        curr_node = n;
        //if(my_rank == n->rank){
        int n_rank = G.get_rank_node(n);
        int n_ln = G.get_local_node_num(n);
        if(my_rank == n_rank) {
          curr_idx = (use_reverse_edge) ? G.local_r_begin[n_ln] : G.local_begin[n_ln];
          curr_end = (use_reverse_edge) ? G.local_r_begin[n_ln + 1] : G.local_begin[n_ln + 1];
          if(use_reverse_edge) {
          curr_idx += G.global_reverse_edge_start_for_all[n_rank];
          curr_end += G.global_reverse_edge_start_for_all[n_rank];
          } else {
          curr_idx += G.global_forward_edge_start_for_all[n_rank];
          curr_end += G.global_forward_edge_start_for_all[n_rank];
          }
        }
        MPI_Bcast(&curr_idx, 1, MPI_INT, n_rank, MPI_COMM_WORLD);
        MPI_Bcast(&curr_end, 1, MPI_INT, n_rank, MPI_COMM_WORLD);
        //and then bcast
        // mark visited
        add_visited(n);
        cnt++;
    ...
    void main_loop() {
        ...
        while (curr_node != INVALID_NODE) {
            ...
            if (curr_idx == curr_end) {
                ...
                //----------------------------------
                node_t z;
                int e_rank = G.get_rank_edge(curr_idx);
                int e_ln = G.get_local_edge_num(curr_idx);
                if(my_rank == e_rank) {
                  if (use_reverse_edge) {
                    z = G.local_r_node_idx[e_ln];
                  } else {
                    z = G.local_node_idx[e_ln];
                  }
                }
                MPI_Bcast(&z, 1, MPI_INT, e_rank, MPI_COMM_WORLD);
                //if(my_rank == curr_idx->rank()) {
                //} and then broad_cast. 
                if (has_visited(z)) {
                    curr_idx++;
                    continue;
