...
class gm_dfs_template
{
  ...
  public:
    gm_dfs_template(gm_graph& _G) :
        ...
        node_t * new_node_idx = (node_t *)calloc(G.num_nodes(), sizeof(node_t));
        node_t * new_begin = (node_t *)calloc(2, sizeof(edge_t));
    }
...
#pragma omp parallel for
    ...
    void enter_node(node_t n) {
        ...
        curr_node = n;
        fprintf(stderr,"use_rev_edge : %d\n", use_reverse_edge);

        if(!use_reverse_edge){
          lock_and_get_begin(new_begin[0], n, 2, G);
          int node_idx_size = new_begin[1] - new_begin[0];
          lock_and_get_node_idx(new_node_idx[0], n, new_begin[0], node_idx_size, G);
          adjust_begin_to_global_values(new_begin, n, G);
        } else {
          lock_and_get_r_begin(new_begin[0], n, 2, G);
          int node_idx_size = new_begin[1] - new_begin[0];
          node_t * new_node_idx = (node_t *)calloc(node_idx_size, sizeof(node_t));
          lock_and_get_r_node_idx(new_node_idx[0], n, new_begin[0], node_idx_size, G);
          adjust_r_begin_to_global_values(new_begin, n, G);
        }
        
        //curr_idx = (use_reverse_edge) ? G.r_begin[n] : G.begin[n];
        //curr_end = (use_reverse_edge) ? G.r_begin[n + 1] : G.begin[n + 1];

        curr_idx = new_begin[n];
        curr_end = new_begin[n+1];
        // mark visited
        add_visited(n);
        cnt++;
    ...
    void main_loop() {
        ...
        while (curr_node != INVALID_NODE) {
            ...
            if (curr_idx == curr_end) {
                ...
                node_t z;
                //if (use_reverse_edge) {
                    z = new_node_idx[curr_idx-new_begin[0]];
                //} else {
