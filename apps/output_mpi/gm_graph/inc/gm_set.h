...
#define lock_and_put_node_property_uchar(_result, _ba_index, _rank, _prop_index)\
{\
    int _err_val;\
    ...
    MPI_Put(&_result, 1, MPI_INT, _rank, base_address[G.get_num_processes()*_ba_index +_rank] + _prop_index*sizeof(unsigned char), 1, MPI_UNSIGNED_CHAR, win);\
...
{
  public:
    gm_node_set(size_t _max_sz, gm_graph& _G) :
            max_sz(_max_sz), G(_G), byte_map(NULL) {
              init();
          // init(setLock);
    }
    ...
    void add_seq(node_t e) {
      if(G.get_rank() == G.get_rank_node(e))
      {
        byte_map[G.get_local_node_num(e)] = 1;
      }
    }

    void add_par(node_t e) {
      int e_rank = G.get_rank_node(e);
      int e_ln = G.get_local_node_num(e);
      int val = 1;
      lock_and_put_node_property_uchar(val, byte_map_index, e_rank, e_ln );
  ...
  private:
    void init()
    {
       ...
       byte_map_index = base_address_index;
       G.win_attach_uchar(byte_map, max_sz);
  ...
  private:
    //gm_node_set() : max_sz(-1),  byte_map(NULL) {
